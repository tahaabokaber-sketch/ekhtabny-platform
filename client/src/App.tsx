...
type StudentView = "dashboard" | "exams" | "history" | "analytics" | "profile" | "details" | "preview" | "taking" | "result";
...
function Toast({ toast, onClose }: { toast: { message: string; type: ToastType } | null; onClose: () => void }) {
  ...
  const icon = toast.type === "success" ? <CheckCircle2 size={18} /> : toast.type === "error" ? <AlertCircle size={18} /> : toast.type === "loading" ? <LoaderCircle className="spin-icon" size={18} /> : <Zap size={18} />;
...
function StudentApp({ view, setView, exams, attempts, selectedExam, setSelectedExam, selectedAttempt, setSelectedAttempt, openExam, saveAttempt, logout, notify }: { view: StudentView; setView: (v: StudentView) => void; exams: Exam[]; attempts: Attempt[]; selectedExam: Exam | null; setSelectedExam: (e: Exam | null) => void; selectedAttempt: Attempt | null; setSelectedAttempt: (a: Attempt | null) => void; openExam: (e: Exam) => void; saveAttempt: (a: Attempt) => void; logout: () => void; notify: (m: string, t?: ToastType) => void }) {
  ...
  const items = [{ id: "dashboard", label: "الرئيسية", icon: House }, { id: "exams", label: "الامتحانات", icon: FileText, badge: "3" }, { id: "history", label: "سجل النتائج", icon: History }, { id: "analytics", label: "تحليلات الأداء", icon: LineChart }, { id: "profile", label: "الملف الشخصي", icon: UserRound }];
  ...
  return <div className="app-frame"><Sidebar active={view} setActive={setView} items={items} mobileOpen={mobileOpen} onClose={() => setMobileOpen(false)} /><div className="app-main"><AppHeader onLogout={logout} onMenu={() => setMobileOpen(true)} title={view === "dashboard" ? "الرئيسية" : items.find(x => x.id === view)?.label} /><main className="page-content">{view === "dashboard" && <StudentDashboard setView={setView} exams={exams} attempts={attempts} openExam={openExam} />}{view === "exams" && <StudentExams exams={exams} openExam={openExam} />}{view === "history" && <StudentHistory attempts={attempts} exams={exams} setSelectedAttempt={(a) => { setSelectedAttempt(a); setSelectedExam(exams.find(e => e.id === a.examId) || exams[0]); setView("result"); }} />}{view === "analytics" && <StudentAnalytics attempts={attempts} exams={exams} />}{view === "profile" && <ProfilePage notify={notify} />}</main></div></div>;
}
...
function StudentAnalytics({ attempts, exams }: { attempts: Attempt[]; exams: Exam[] }) {
  const subjectRows = [
...
function TakingExam({ exam, setView, saveAttempt, notify }: { exam: Exam; setView: (v: StudentView) => void; saveAttempt: (a: Attempt) => void; notify: (m: string, t?: ToastType) => void }) {
  const [current, setCurrent] = useState(0); const [answers, setAnswers] = useState<Record<number, string>>({}); const [marked, setMarked] = useState<number[]>([]); const [seconds, setSeconds] = useState(exam.duration * 60); const [confirm, setConfirm] = useState(false); const [isSubmitting, setIsSubmitting] = useState(false);
